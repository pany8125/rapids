...
public interface AdminRoleRepo extends PagingAndSortingRepository<AdminRole, Long> {
    ...
    @Query(value = "SELECT distinct c.id as id, a.name as name, a.description as  description, a.status as status FROM AdminRole a, AdminRoleMember c where a.id = c.roleid and c.adminid=?1", nativeQuery = true)
